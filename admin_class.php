...
Class Action {
	...
	function save_filter(){
		...
		$data = " filter = '$name' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO filters set $data");
			}else{
				$save = $this->db->query("UPDATE filters set $data where id = $id");
			}
		if($save)
			return 1;
	}
	...
	function delete_filter(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM filters where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_topic(){
		...
		$bad_words = $this->db->query("SELECT filter FROM filters");
		...
		$data .= ", user_id = '{$_SESSION['login_id']}' ";
			...
			if ($save)	{
				move_uploaded_file($_FILES['upload']['tmp_name'], "$files");
			}
		}else{
